...
class Parser {
  ...
  private Stmt forStatement() {
    ...
    if (increment != null) {
      Vector<Stmt> addIncrement = new Vector<Stmt>();
      addIncrement.addElement(body);
      addIncrement.addElement(new Stmt.Expression(increment));
      body = new Stmt.Block(addIncrement);
    }
    ...
    if (initializer != null) {
      Vector<Stmt> addInit = new Vector<Stmt>();
      addInit.addElement(initializer);
      addInit.addElement(body);
      body = new Stmt.Block(addInit);
    }
    return body;
  }
  ...
  private Stmt classDeclaration() {
    ...
    Vector<Stmt.Function> methods = new Vector<>();
    // isAtEnd check is done to prevent infinite loops if no } is found
  ...
  private Expr finishCall(Expr callee) {
    ...
    if (!check(RIGHT_PAREN)) {
      do {
        // this conditional is check each time
  ...
  private Expr call() {
    ...
    while (true) {
      if (match(LEFT_PAREN)) {
        ...
        Token name =
            consume(IDENTIFIER, "Expect property name after dot accessor.");
